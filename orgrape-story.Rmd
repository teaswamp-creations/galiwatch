...
ep <- read_sheet(ep_id) %>%
  ...
  mutate(Time = hms::as_hms(paste0(hour(Time), ':00:00'))) %>%
  group_by(Date)
...
df <- tibble(merge(ep, ws, c('Date', 'Time')))
...
### Life Cycle {data-commentary-width=450}

![](static/orgrape_growing.gif)
...
### Phenology – biological events that depend on climate {data-commentary-width=450}


![](static/orgrape_timeline.png){width=100%}
...
### Weather {data-commentary-width=450}
...
ws %>% 
  ...
  add_ribbons(x = ~Date, 
              text = round(summarize(
                ws, lo=min(`Outdoor Temperature C`))$lo, 2),
...
### Air Quality {data-commentary-width=450}
...
### Flower-insect timed (FIT) counts {data-commentary-width=600}

```{r}
readr::read_csv('static/fit_042021.csv') %>%
...
### Pollenator counts from video footage {data-commentary-width=600}
...
Video footage was collected from April 21st - May 4th. Our cameras were det to collect 4 min of footage every 5 min. Video counts provide an estimate of the proportion of pollinators seen in time lapse pictures. 
...
### Pollinator visits during peak blooming {data-commentary-width=600}
