...
But we'll save the details for a future blog post `r emo::ji('smile')`. For now, lets dive in to some of the data we've collected!


# Setup

We'll read in our data: weather station data, and the bounding boxes of detected birds. 
...
p2 <- bird %>% 
  ...
  ggplot() + 
  aes(x = Date, fill = Sex, y=after_stat(count)) + 
  geom_density(bw=10, position='fill',  colour =NA) + 
  labs(y='Proportion of IDs') + 
...
merge(bird, weather, all.x=T) %>%
  ...
  ggplot() + 
  geom_bar(aes(x=factor(Hour), fill = Sex)) +
  geom_point(aes(x=Hour-5, y=`Temp C`, color=Sex)) +
  scale_y_continuous(name = "N birds", sec.axis = sec_axis(~./10,name="Temp C"))+
  facet_wrap("~Month", scales='free') + 
  scale_fill_manual(values=c('chocolate1', 'chocolate4')) + 
  labs(title='Rufous visit times by sex and hour per month', y = 'proportion') + 
  theme_pubr()
...
p2 <- bird %>%
  ...
  geom_bar(position='fill') +facet_wrap("~Month") + 
  scale_fill_manual(values=c('chartreuse1', 'chartreuse4')) + 
  labs(title='Annas visit times by sex and hour per month', y = 'proportion') + 
  theme_pubr()
