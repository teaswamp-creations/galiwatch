...
+	`r pop_gloss('Convoultional neural network')` and `r pop_gloss('Movidius NCS2')` for applying `r pop_gloss('Deep learning')` for insect detection and image screening  


 We have mounted two surveillance cameras comprising of a Raspberry Pi (powered over ethernet) fitted with a Pi camera and a 1 terabyte `r pop_gloss('SSD drive')`, inside a waterproof box. The cameras have been situated so that they point down towards the patches of salal and dull Oregon grape. By positioning the cameras about a meter from the tops of the plants, we can film an area of about half a square meter with high resolution. The cameras are set to take a picture every minute and the images are stored on the SSD drive.
